...
{
	Stack <Lex, 100> args;
	int tid_pos, index=0, size=prog.get_free();
	double creal1, creal2;
	int cint1, cint2;
	...
	while (index<size)
	{
		...
		switch (curr_lex.get_type())
		{
			...
			case LEX_EQUAL:
				upper_op=args.pop();
				if (upper_op.get_type()==LEX_ID)
				{
					tid_pos=upper_op.get_value();
					if (TID[tid_pos].get_type()==LEX_INT)
					{
						...
						if (TID[tid_pos].get_type()==LEX_INT)
						{
							TID[tid_pos].put_int(cint1);
						}
						else if (TID[tid_pos].get_type()==LEX_REAL)
						{
							TID[tid_pos].put_real(cint1);
						}
					}
					else if (TID[tid_pos].get_type()==LEX_REAL)
					{
						creal1=TID[tid_pos].get_real();
						curr_lex=args.pop();
						tid_pos=curr_lex.get_value();
						if (TID[tid_pos].get_type()==LEX_INT)
						{
							TID[tid_pos].put_int(creal1);
						}
						else if (TID[tid_pos].get_type()==LEX_REAL)
						{
							TID[tid_pos].put_real(creal1);
						}
					}
					else if (TID[tid_pos].get_type()==LEX_STRING)
					{
						cstring1=TID[tid_pos].get_string();
						curr_lex=args.pop();
						tid_pos=curr_lex.get_value();
						TID[tid_pos].put_string(cstring1);
					}
				}
				else if (upper_op.get_type()==LEX_CREAL)
				{
					curr_lex=args.pop();
					tid_pos=curr_lex.get_value();
					if (TID[tid_pos].get_type()==LEX_INT)
					{
						TID[tid_pos].put_int(upper_op.get_real());
					}
					else if (TID[tid_pos].get_type()==LEX_REAL)
					{
						TID[tid_pos].put_real(upper_op.get_real());
					}
				}
				else if (upper_op.get_type()==LEX_CINT)
				{
					curr_lex=args.pop();
					tid_pos=curr_lex.get_value();
					if (TID[tid_pos].get_type()==LEX_INT)
					{
						TID[tid_pos].put_int(upper_op.get_int());
					}
					else if (TID[tid_pos].get_type()==LEX_REAL)
					{
						TID[tid_pos].put_real(upper_op.get_int());
					}
				}
				else if (upper_op.get_type()==LEX_CSTRING)
				{
					curr_lex=args.pop();
					tid_pos=curr_lex.get_value();
					TID[tid_pos].put_string(upper_op.get_string());
				}
				args.push(curr_lex);
				break;	
			case LEX_READ:
				...
				{
					throw "Логическая ошибка в интерпретаторе (при read): эта ошибка не может быть выдана при корректной работе";
				}
				break;	
			case LEX_WRITE:
				curr_lex=args.pop();
				if (curr_lex.get_type()==LEX_ID)
				{
					...
					if (TID[tid_pos].get_type()==LEX_INT)
					{
						std::cout << TID[tid_pos].get_int() << std::endl;
					}
					else if (TID[tid_pos].get_type()==LEX_REAL)
					{
						std::cout << TID[tid_pos].get_real() << std::endl;
					}
					else if (TID[tid_pos].get_type()==LEX_STRING)
					{
						std::cout << TID[tid_pos].get_string() << std::endl;
					}
				}
				else if (curr_lex.get_type()==LEX_CINT)
				{
					std::cout << curr_lex.get_int() << std::endl;
				}
				else if (curr_lex.get_type()==LEX_CREAL)
				{
					std::cout << curr_lex.get_real() << std::endl;
				}
				else if (curr_lex.get_type()==LEX_CSTRING)
				{
					std::cout << curr_lex.get_string() << std::endl;
				}
				args.push(curr_lex);
				break;
			case LEX_OR:
